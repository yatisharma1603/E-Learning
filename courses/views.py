...
########################################################################
'''
class AssignmentUploadView(TemplateResponseMixin, View):
    template_name = 'courses/manage/course/form_assign.html'
    course = None
    # to avoid repeating the code to build the formset
    def get_formset(self, data=None):
        return ModuleFormSet(instance=self.course, data=data)
    # http handeling
    def dispatch(self, request, pk):
        self.course = get_object_or_404(Assignments,
                                        id=pk,
                                        owner=request.user)
        return super(AssignmentUploadView, self).dispatch(request, pk)
    # executed for get request
    def get(self, request, *args, **kwargs):
        formset = self.get_formset()
        return self.render_to_response({'course': self.course,
...
class AssignmentMixin(OwnerMixin, LoginRequiredMixin):
    model = Assignments
    fields = ['subject', 'title', 'slug', 'overview']
...
class AssignListView(ListView):
    model = Assignments
    template_name = 'form_assign.html'
    #template_name = 'courses/manage/course/form_assign.html'
    #success_url = reverse_lazy('assignment_detail')
'''
######################################################################
class OwnerAssignmentsEditMixin(OwnerCourseMixin, OwnerEditMixin):
    fields = ['subject', 'title', 'slug', 'overview']
...
class AssignmentsCreateView(PermissionRequiredMixin,
                        OwnerAssignmentsEditMixin, CreateView):
...
#    print(request.GET)
#    return render(request, "form_assign.html")
